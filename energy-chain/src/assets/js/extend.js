        ...
        for (var prop in src) {
            if (src.hasOwnProperty(prop)) {
                if(typeof (src[prop]) == 'function'){
                    dst[prop] = yjys.JSON.decode(src[prop].toString());
                }
                else if((src[prop] instanceof Object || src[prop] instanceof Array) ){
                    dst[prop] = yjys.copyFull(src[prop]);
                }else{
                    dst[prop] = src[prop];
                }
            }
...
yjys.disTable.prototype = {
    ...
    getColumnObjList:function () {
        var columnList = this.configInfo.defaultHeaderInfo;
        var dynamicCols = this.configInfo.visualHeaderInfo.visualHeads;
    ...
    getVisualHeadDataList:function () {
        // 处理伪表头
        var tmpDataList = [];
        var tmpHeads = this.configInfo.visualHeaderInfo.visualHeads;
        var tmpColumnList = this.configInfo.defaultHeaderInfo;
        var tmpReadDataList = this.configInfo.dataList;
        for(var i = 0;i< tmpHeads.length - 1;i++){
            tmpDataList.push({});
        }
        ...
        if(!yjys.isEmpty(tmpReadDataList)){
            for(var i in tmpReadDataList){
                var tmpRawData = tmpReadDataList[i];
                var tmpTestNums = tmpRawData.TestNums;
                tmpDataList.push(tmpRawData);
                if(!yjys.isEmpty(tmpTestNums) && tmpTestNums > 1){
                    for(var a=0;a< tmpTestNums - 1;a++){
                        tmpDataList.push();
                    }
                }
            }
    ...
    getMergeInfo:function(){
        ...
        var tmpVisualMergeCells = this.configInfo.visualHeaderInfo.visualHeadMerges;
        var tmpHeads = this.configInfo.visualHeaderInfo.visualHeads;
        var tmpColumnList = this.configInfo.defaultHeaderInfo;
        var tmpRealDataMerges = this.configInfo.visualHeaderInfo.realDataMerges;
        if(tmpHeads.length == 3){
            for(var i in tmpColumnList){
                var tmp = yjys.copyDeep(tmpColumnList[i]);
        ...
        for(var i in tmpVisualMergeCells){
            var tmp = yjys.copyDeep(tmpVisualMergeCells[i]);
        ...
        for(var i in tmpRealDataMerges){
            var tmp = yjys.copyDeep(tmpRealDataMerges[i]);
            if(typeof(tmp) == "function"){
                continue;
            }
            tmpMergeCells.push(tmp);
     ...
     * 获取数据合并信息
     */
    getReBulidMergeInfo:function () {
        var tmpRawDataList =  this.configInfo.rawDataList;
        var tmpHeadCounts = this.configInfo.visualHeaderInfo.visualHeads.length - 1;
        var tmpDataMergeList = [];
        var tmpVisualHeadMerges = this.getMergeInfo();
        var tmpMergeColIdxs = this.configInfo.visualHeaderInfo.mergeColIdxs;
        for(var i in tmpVisualHeadMerges){
            tmpDataMergeList.push(yjys.copyDeep(tmpVisualHeadMerges[i]));
        }
        var tmpDataIdx = tmpHeadCounts;
        var tmpRowSpan = 0;
        for(var i=0; i< tmpRawDataList.length;i++){
            // if(i < tmpHeadCounts){
            //      continue;
            // }
            var tmp = tmpRawDataList[i];
            tmpDataIdx = tmpDataIdx + tmpRowSpan;
            tmpRowSpan = tmp.TestNums;
            if(tmpRowSpan<=1){
                continue;
            }
            for(var a =0;a<tmpMergeColIdxs.length;a++) {
                tmpDataMergeList.push({
                    "row": tmpDataIdx,
                    "col": tmpMergeColIdxs[a],
                    "rowspan": tmpRowSpan,
                    "colspan": 1
                });
            }
        ...
        return tmpDataMergeList;

    },
    
    furshTableData : function (tableObj) {
        var tmpTableData = this.getHandsonTableFullData(tableObj);
        this.configInfo.dataList=[];
        for(var i =this.configInfo.visualHeaderInfo.visualHeads.length - 1;i<tmpTableData.length;i++){
            this.configInfo.dataList.push(tmpTableData[i]);
        }
        tableObj.updateSettings({"mergeCells":this.getReBulidMergeInfo()});
    },
    /**
     * 新增实验次数
     * @param tableObj 表格对象
     * @param times 增加次数
     */
    addTestTimes:function (tableObj,times) {
        var tmpMaxTestCounts = yjys.isEmpty(this.configInfo.maxTestCounts) ? 10 : this.configInfo.maxTestCounts;
        times = times > tmpMaxTestCounts - 1 ? tmpMaxTestCounts - 1 : times;
        var tmpDataList = this.getNewSourceDataAndMergeList(tableObj,times);
        if(yjys.isEmpty(tmpDataList))
            return;
        //tableObj.loadData(tmpDataList);
        tableObj.updateSettings(tmpDataList);
    },
    /**
     * 新增实验次数
     * @param tableObj 表格对象
     * @param times 增加次数
     */
    delTestTimes:function (tableObj,times) {
        var tmpMaxTestCounts = yjys.isEmpty(this.configInfo.maxTestCounts) ? 10 : this.configInfo.maxTestCounts;
        times = times > tmpMaxTestCounts - 1 ? tmpMaxTestCounts - 1 : times;
        var tmpDataList = this.getNewSourceDataAndMergeList(tableObj,times);
        if(yjys.isEmpty(tmpDataList))
     ...
     * 获取当前行数据
     */
    getNewSourceDataAndMergeList : function (tableObj,times) {
        var tmpMaxTestCounts = yjys.isEmpty(this.configInfo.maxTestCounts) ? 10 : this.configInfo.maxTestCounts;
        var tmpSelectCell = tableObj.selectCell;
        var tmpHeadCounts = this.configInfo.visualHeaderInfo.visualHeads.length - 1;
        if(yjys.isEmpty(tmpSelectCell) || yjys.isEmpty(tmpSelectCell.row) || tmpSelectCell.row < tmpHeadCounts){
            return;
        }
        var tmpCurrentRowIdx = tmpSelectCell.row;//tableObj.getSelectd()[0];
        var tmpSourceDataList = this.getHandsonTableFullData(tableObj);
        var tmpCurrentRowData = tmpSourceDataList[tmpCurrentRowIdx];
        var tmpIdxColumnName = this.configInfo.visualHeaderInfo.idxColumnName;
        var tmpDataInsertIndex = this.getDataInsertIndex(tmpCurrentRowData);
        var tmpDataGroupLength = this.getDataGroupLength(tmpCurrentRowData,times);
        var tmpDataIndex = this.getDataIndex(tmpCurrentRowData);

        var tmpRealTimes = 0;
        var tmpVisualHeads = this.configInfo.visualHeaderInfo.visualHeads[this.configInfo.visualHeaderInfo.visualHeads.length - 1];
        if(times<0){
            for(var a=0;a>times;a--){
                if(this.getDataGroupLength(tmpCurrentRowData) <=1){
                    continue;
                }
                tmpSourceDataList.splice(tmpDataInsertIndex + a - 1,1);
                this.setDataGroupLength(tmpCurrentRowData,-1);
                tmpDataIndex = this.getDataIndex(tmpCurrentRowData);
                tmpRealTimes--;
            }
        } else{
            for(var a=0;a<times;a++){
                if(this.getDataGroupLength(tmpCurrentRowData) >= tmpMaxTestCounts){
                    continue;
                }
                var tmpNewRowData = yjys.copyDeep(tmpCurrentRowData);
                tmpNewRowData[tmpIdxColumnName] = tmpDataGroupLength+a+1;
                for(var i in tmpVisualHeads){
                    tmpNewRowData[tmpVisualHeads[i]] = null;
                }
                if(tmpDataInsertIndex > 0){
                    tmpSourceDataList.splice(tmpDataInsertIndex+a,0,tmpNewRowData);
                }
                this.setDataGroupLength(tmpCurrentRowData,1);
                tmpDataIndex = this.getDataIndex(tmpCurrentRowData);
                tmpRealTimes++;
        ...
        // var tmpMergeList = this.getDataMergeInfo(tableObj,tmpRealTimes,tmpDataInsertIndex-(Math.abs(times)-Math.abs(tmpRealTimes)),tmpDataIndex);
        var tmpMergeList = this.getReBulidMergeInfo();
        return {"data":tmpSourceDataList,"mergeCells":tmpMergeList,"maxRows":tmpSourceDataList.length};
    },
    /**
     * 获取新增数据行号
     * @param currentRowData 当前行数据
     * @returns {number} 新增数据行号
     */
    getDataInsertIndex : function (currentRowData) {
        var tmpPrimaryKeyName = this.configInfo.visualHeaderInfo.primaryKeyName;
        var tmpRawDataList =  this.configInfo.rawDataList;
        var tmpIndex = 0;
        var tmpHeadCounts = this.configInfo.visualHeaderInfo.visualHeads.length - 1;
        for(var i in tmpRawDataList){
            var tmp = tmpRawDataList[i];
            tmpIndex = tmpIndex + tmp.TestNums;
            if(tmp[tmpPrimaryKeyName] === currentRowData[tmpPrimaryKeyName]){
                return tmpIndex+tmpHeadCounts;
            }
        }
        return -1;
    },
     ...
     * @returns {number} 新增数据行号
     */
    getDataIndex : function (currentRowData) {
        var tmpPrimaryKeyName = this.configInfo.visualHeaderInfo.primaryKeyName;
        var tmpRawDataList =  this.configInfo.rawDataList;
        var tmpIndex = 0;
        var tmpHeadCounts = this.configInfo.visualHeaderInfo.visualHeads.length - 1;
     ...
     * @returns {number} 新增数据行号
     */
    getDataGroupLength : function (currentRowData) {
        var tmpPrimaryKeyName = this.configInfo.visualHeaderInfo.primaryKeyName;
        var tmpRawDataList =  this.configInfo.rawDataList;
        var tmpTestNums = 0;
        for(var i in tmpRawDataList){
     ...
     * @returns {number} 新增数据行号
     */
    setDataGroupLength : function (currentRowData,times) {
        var tmpPrimaryKeyName = this.configInfo.visualHeaderInfo.primaryKeyName;
        for(var i in this.configInfo.rawDataList){
            if(this.configInfo.rawDataList[i][tmpPrimaryKeyName] === currentRowData[tmpPrimaryKeyName]){
                this.configInfo.rawDataList[i].TestNums = this.configInfo.rawDataList[i].TestNums + times;
                return;
            }
        }
    },
    ...
    getRawCalcValue:function (dataInfo) {
        ...
        var tmpScopeStr = tmpScopeConfigStr[0];
    ...
    getReportData : function (tableObj,countType) {
        var tmpTableData = this.getHandsonTableFullData(tableObj)
        // var tmpPrimaryKeyName = this.configInfo.visualHeaderInfo.primaryKeyName;
        var tmpRawDataList =  this.configInfo.rawDataList;
        this.configInfo.dataList = tmpTableData;
        var tmpIdx = this.configInfo.visualHeaderInfo.visualHeads.length -1;
        for(var i in tmpRawDataList){
            var tmp = tmpRawDataList[i];
            switch (countType){
                case "max":
                    tmp[this.configInfo.visualHeaderInfo.reportColName] = this.getMaxReportData(tmpTableData,tmpIdx,tmp["TestNums"]);
                    for(var j=0;j<tmp["TestNums"];j++){
                        tmpTableData[tmpIdx+j][this.configInfo.visualHeaderInfo.reportColName] = tmp[this.configInfo.visualHeaderInfo.reportColName];
                    }
                    break;
                case "min":
                    tmp[this.configInfo.visualHeaderInfo.reportColName] = this.getMinReportData(tmpTableData,tmpIdx,tmp["TestNums"]);
                    for(var j=0;j<tmp["TestNums"];j++){
                        tmpTableData[tmpIdx+j][this.configInfo.visualHeaderInfo.reportColName] = tmp[this.configInfo.visualHeaderInfo.reportColName];
                    }
                    break;
                case "avg":
                    tmp[this.configInfo.visualHeaderInfo.reportColName] = this.getAvgReportData(tmpTableData,tmpIdx,tmp["TestNums"]);
                    for(var j=0;j<tmp["TestNums"];j++){
                        tmpTableData[tmpIdx+j][this.configInfo.visualHeaderInfo.reportColName] = tmp[this.configInfo.visualHeaderInfo.reportColName];
                    }
                    break;
            ...
            tmpIdx = tmpIdx +  tmp["TestNums"];
        }
        var tmpMergeList = this.getReBulidMergeInfo();
        var tmpDataList = {"data":tmpTableData,"mergeCells":tmpMergeList,"maxRows":tmpTableData.length};
        if(yjys.isEmpty(tmpDataList))
            return;
        tableObj.updateSettings(tmpDataList);
    },
    getAvgReportData : function (tmpTableData,startIdx,TestNums) {
        var tmpVal = 0.0;
        for(var i =startIdx;i< startIdx+TestNums;i++){
            tmpVal = tmpVal + this.getRawCalcValue(tmpTableData[i]);
        }
        return (tmpVal*1.0) / TestNums;
    },
    getMaxReportData : function (tmpTableData,startIdx,TestNums) {
        var tmpVal = Math.max();
        for(var i =startIdx;i< startIdx+TestNums;i++){
            if(this.getRawCalcValue(tmpTableData[i])>tmpVal){
                tmpVal = this.getRawCalcValue(tmpTableData[i]);
            }
        }
        return tmpVal == Math.max() ? '/' : tmpVal * 1.0;
    },
    getMinReportData : function (tmpTableData,startIdx,TestNums) {
        var tmpVal = Math.min();
        for(var i =startIdx;i< startIdx+TestNums;i++){
            if(this.getRawCalcValue(tmpTableData[i])<tmpVal){
                tmpVal = this.getRawCalcValue(tmpTableData[i]);
        ...
        return tmpVal == Math.min() ? '/' : tmpVal * 1.0;
    },
    ...
    getHandsonTableData: function (tableObj) {
        ...
        for (var i = this.configInfo.visualHeaderInfo.visualHeads.length - 1; i < rows; i++) {
            var tmpData = {};
    ...
    getHandsonTableFullData: function (tableObj) {
        var tmpDatas = [];
        var columns = this.getColumnObjList();
        if (yjys.isEmpty(tableObj) || yjys.isEmpty(columns)) {
            return null;
    ...
    getConfigInfo:function () {
        return yjys.copyDeep(this.configInfo);
    }
};
...
yjys.makeMasterSlaveArray = function (rawList,primaryKey,parentKey,childKey,rootNodeVal) {
    var tmpResultList = [];
    for(var i in rawList){
        ...
        var tmp = rawList[i];
        if(!yjys.isEmpty(tmp) && tmp[parentKey] == rootNodeVal){
            tmpResultList.push(tmp);
        }
    }
    for(var i in tmpResultList){
        if(!tmpResultList.hasOwnProperty(i)){
            continue;
        }
        var tmp = tmpResultList[i];
        tmp[childKey] = yjys.makeMasterSlaveArray(rawList,primaryKey,parentKey,childKey,tmp[primaryKey]);
    }
 ...
 */
yjys.Calculate.pearsonCorrelation = function (prefs, p1, p2) {
    var si = [];
...
yjys.getAttrFromObj = function (obj,attrName) {
    if(yjys.isEmpty(obj) || yjys.isEmpty(attrName)){
        return null;
    }

    var tmpAttrList = attrName.split('.');
    var tmpVal = obj;
    for (var tmp  in tmpAttrList){
        tmpVal = tmpVal[tmpAttrList[tmp]];
    }
    return tmpVal;
}
...
yjys.String={
    lpad:function(str,completeChar,length){
        var tmpStr = String(str);
        var tmpCompleteChar = String(completeChar);
        var tmpLength = tmpStr.length;
        if(tmpLength<length){
            for(var i=0;i<length-tmpLength;i++){
                tmpStr = completeChar + tmpStr;
            }
        }
        return tmpStr;
    },
    rpad:function(str,completeChar,length){
        var tmpStr = String(str);
        var tmpCompleteChar = String(completeChar);
        var tmpLength = tmpStr.length;
        if(tmpLength<length){
            for(var i=0;i<length-tmpLength;i++){
                tmpStr = tmpStr+completeChar;
            }
        }
        return tmpStr;
    }
...
// yjys.SetDictValue = function (dictList,formData,column) {
//     for(var i=0;i<formData.length;i++){
//         formData[i][column]= dictList[formData[i][column]];
//     }
// }
